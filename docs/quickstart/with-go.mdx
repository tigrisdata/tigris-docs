...
type User struct {
	UserId  int32 `tigris:"primary_key,autoGenerate"`
	Name    string
	Balance float64
}
...
func main() {
	...
	// Read
	var user *User
	user, err = users.ReadOne(ctx, filter.Eq("UserId", 1)) // find user with primary key field UserId
	if err != nil {
		panic(err)
	}
	fmt.Println(user)
	...
	users.Update(ctx, filter.Eq("UserId", 1), fields.Set("Name", "Jania McGrover"))

	// Transaction - transfer balance between users
	db.Tx(ctx, func(txCtx context.Context) error {
		var userOne *User
		if userOne, err = users.ReadOne(txCtx, filter.Eq("UserId", 1)); err != nil {
			return err
		}

		var userTwo *User
		if userTwo, err = users.ReadOne(txCtx, filter.Eq("UserId", 2)); err != nil {
			return err
		}

		if _, err = users.Update(txCtx, filter.Eq("UserId", 1),
			fields.Set("Balance", userOne.Balance-100)); err != nil {
			return err
		}
		if _, err = users.Update(txCtx, filter.Eq("UserId", 2),
	...
	// Delete - delete users with UserId 1 or 2
	users.Delete(ctx, filter.Or(filter.Eq("UserId", 1), filter.Eq("UserId", 2)))
