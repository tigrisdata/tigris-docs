...
public class TigrisApp {
    ...
    public static void main(String[] args) throws TigrisException {
        ...
        int janiaId = user1.getId();
        int bunnyId = user2.getId();

        // read user back
        User user = users.readOne(Filters.eq("id", janiaId)).get();

        // update
        users.update(
                Filters.eq("id", janiaId),
        ...
        helloDB.transact(
                (tx) -> {
                    try {
                        User jania = users.readOne(tx, Filters.eq("id", janiaId)).get();
                        User bunny = users.readOne(tx, Filters.eq("id", bunnyId)).get();
        ...
        // delete jania and bunny
        users.delete(Filters.eq("id", janiaId));
        users.delete(Filters.eq("id", bunnyId));
    }
